...
def print_draw_crop_rec_res( img_crop_list, img_name):
        ...
        for bno in range(bbox_num):
          ...
          crop_name_w = "./img_crop/{}".format(crop_name)
          cv2.imwrite(crop_name_w, img_crop_list[bno])
          crop_label.write("{0}\t{1}\n".format(crop_name, text[bno]))

crop_label = open('./crop_label.txt','w')
with open('./train_label.txt','r') as file_text:
...
for img_file in img_files:
  ...
  image_file = './vietnamese_original/vietnamese/train_images/' +img_file.split('\t')[0]
  ...
  if not os.path.exists('./img_crop'):
    os.mkdir('./img_crop') 
